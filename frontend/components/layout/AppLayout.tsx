...
export default function AppLayout({ children }: AppLayoutProps) {
  ...
  useEffect(() => {
    ...
    setUser(u)
    // Storage usage removed — use DDMS quota endpoint when available
  }, [router])

  useEffect(() => {
    const handleToggle = () => setSideOpen(prev => !prev)
    globalThis.window?.addEventListener('toggle-sidebar', handleToggle)
    return () => globalThis.window?.removeEventListener('toggle-sidebar', handleToggle)
  }, [])

  useEffect(() => {
    const isInvestor = user?.scope === 'investor'
    const targetPath = isInvestor ? '/dashboard/investor' : '/dashboard/files'
  ...
  const handleSearchChange = (val: string) => {
    setSearchVal(val)
    if (globalThis.window !== undefined) {
      ;(globalThis.window as any).__searchQuery = val
      globalThis.window.dispatchEvent(new CustomEvent('global-search', { detail: val }))
    }
    const isInvestor = user?.scope === 'investor'
    const targetPath = isInvestor ? '/dashboard/investor' : '/dashboard/files'
    if (pathname !== targetPath) {
  ...
  const isFilesPage = pathname === '/dashboard/files' || pathname === '/dashboard/investor'
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-white">
        ...
        <div className="h-[56px] sm:h-[64px] flex items-center px-3 sm:px-6 gap-2 sm:gap-4">
          ...
          <div className="hidden sm:flex items-center gap-2.5 px-3 py-1.5 rounded-full bg-[#f1f3f4] hover:bg-[#e8eaed] transition-colors cursor-pointer flex-shrink-0">
            <div className="w-7 h-7 rounded-full bg-gradient-to-br from-[#007aff] via-indigo-500 to-purple-600 flex items-center justify-center text-white text-[13px] font-bold flex-shrink-0">
              {(user?.company_legal_name || user?.username || 'U')[0]?.toUpperCase()}
            </div>
            <span className="text-[14px] font-semibold text-[#333333] pr-1">
              {user?.company_legal_name || user?.username || 'User'}
            </span>
          </div>
        </div>
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
          ...
          <div className="flex lg:hidden items-center justify-between px-4 py-4 border-b border-[#f0f0f0] bg-white">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-[#929292] leading-tight">
                  {user?.perm?.admin ? 'Administrator' : 'Member'}
                </p>
              </div>
            </div>
        ...
        {/* Overlay mobile */}
          ...
          <button
            type="button"
            className="fixed inset-0 z-40 bg-black/20 lg:hidden cursor-default"
