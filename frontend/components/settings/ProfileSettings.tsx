...
export default function ProfileSettings({ currentUser }: ProfileSettingsProps) {
  ...
  const loadProfile = async () => {
    ...
    try {
      const prefStr = localStorage.getItem(`pref_${currentUser.id}`)
      if (prefStr) {
        const pref = JSON.parse(prefStr)
        setHideDotfiles(!!pref.hideDotfiles)
        setSingleClick(!!pref.singleClick)
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem(`pref_${currentUser.id}`, JSON.stringify(payload))
