...
import { getUser, formatBytes } from '@/lib/api'
...
export default function SharedPageContent() {
  ...
  // Load roots using react-query
  const { data: investorRoots, isLoading: loadingInvestor, error: errorInvestor } = 
    useDDMSInvestorIssuerRoots(issuerId, !!user && isInvestor)
  
  const { data: issuerRoots, isLoading: loadingIssuer, error: errorIssuer } = 
    useDDMSRoots(!!user && !isInvestor)

  const loading = isInvestor ? loadingInvestor : loadingIssuer
  const error = isInvestor ? errorInvestor : errorIssuer

  const rawItems = isInvestor ? investorRoots : issuerRoots
  const items: FileItem[] = (rawItems || []).map((f: any) => ({
    path: f.id,
    name: f.name,
    isDir: true,
    size: f.size || 0,
      ...
      <div className="flex-1 overflow-auto p-6">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-64 text-red-500">
            <p>Failed to load shared items.</p>
          </div>
