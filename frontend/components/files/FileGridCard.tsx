...
export default function FileGridCard({
  ...
  return (
    ...
    >
      ...
      {/* Root/Public/Private Badge */}
      {isInvestor && item.isRoot && item.rootType && (
        <span className={clsx(
          'absolute top-2.5 right-2.5 text-[8px] px-1.5 py-0.5 rounded-full font-semibold flex-shrink-0 border uppercase tracking-wider',
          isSelected 
            ? 'bg-white/70 text-[#0062cc] border-transparent' 
            : item.rootType === 'public'
